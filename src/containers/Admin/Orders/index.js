...
function Orders () {
  const [orders, setOrders] = useState([]) // guarda todos os pedidos sempre
  const [filteredOrders, setFilteredOrders] = useState([])
  const [activeStatus, setActiveStatus] = useState(0) // armazenar o status selecionado para estilizaçao
  ...
  useEffect(() => {
    async function loadOrders () {
      ...
      // acrescentando a opção TODOS no menu
        ...
        <Container>
            <Menu>
                {status && status.map(status => (
                    ...
                    isActiveStatus ={activeStatus === status.id}>{status.label}
                    </LinkMenu>
                ))}
